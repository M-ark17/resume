...
\setlist[itemize,1]{label=$\bullet$,leftmargin=4mm,topsep=2mm,itemsep=1mm,itemindent=0mm,parsep=-0.5mm}
...
%
%
\textbf{Completed work:} Implemented \textbf{Multi Cell MMSE based MIMO precoding} in multiple antennas cellular systems which used non-orthogonal pilots for channel estimation.
...
%
\textbf{Ongoing work:} Working on implementation a basic precoder which utilises the information obtained from \textbf{Kalman} estimate in \textbf{coordinated MIMO} systems.
...
\item \textbf{Image Dehazing using colour attenuation prior and dark channel prior 
} \hfill{(\textit{Jul'18 - Nov'18})}\\
Guide: \textit{Prof. Amit Sethi, Electrical Engineering, IIT Bombay}
	\begin{itemize}
	\item Implemented \textbf{Colour Attenuation Prior and Dark Channel Prior} techniques to estimate the Depth map.
...
\hfill{(\textit{Jul'17 - Present})}
	...
	\item Provide mail service, storage space, computing facilities and network facilities to the department.
